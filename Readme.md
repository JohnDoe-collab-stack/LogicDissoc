...
that is, an element of `ℕ^B` (in the code: `GenCounters B`).
...
In the code, such an object is packaged as `LegitObstruction B`, with `L.O.F = F`.

---

## 3. Admissible counting scheme

Everything remains purely semantic (everything goes via `ModE` and `Cons_S`).
...
In other words, the zero profile (all components zero) is equivalent to semantic conservativity.
...
In the code: `AstarGen (Sat := Sat) (Γ_ref := Γ_ref) (L := L) (Count := Count) S`.

---

## 4. Gödel directions (dynamic layer)

The “dynamics” enters through semantic directions that detect which models are lost when the theory is extended.
...
This is exactly what is proved by the `CountSpec` instance for `CountFromGodel`.

One can then specialize the index.
...
2. Strictly positive boundary:
   ...
   Every non-conservative extension is detected and strictly separated from 0 by `A*_Godel`.
...
3. Independence from the choice of L:
   ...
   The qualitative verdict “0 / > 0” does not depend on which linear functional is chosen in the cone of legitimate obstructions.

In the Lean code, this corresponds to

* `metaGodel_frontier_zero` and `metaGodel_frontier_pos` (part `MetaGodel`),
* the `CountSpec` instance for `GodelDirection.CountFromGodel`,
* and the standard lemmas on `LegitObstruction` (non-negativity, trivial kernel).
...
2. **Level 1 – Obstruction cone (geometry on ℕ^B).**
   Legitimate obstructions `L`: positive linear functionals on `ℕ^B` with trivial kernel. This is the geometric layer.
...
This scheme is formulated entirely at the level of model theory (classes of models, conservativity) and linear structure on `ℕ^B`, without committing to any particular syntax or proof calculus.
